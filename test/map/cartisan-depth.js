import * as WT from '../../dist/world-tree.js';

console.log("begin");

let sheet = new WT.SpriteSheet("../res/img/cartisan-grass-floor");

sheet.image.onload = function() {
  let cellsX = 6;
  let cellsY = 6;
  let tileWidth = 64;
  let tileHeight = 64;
  let sprites = [];

  // The sprite sheet is 3x3...
  for (let y = 0; y < 3; y++) {
    for (let x = 0; x < 3; x++) {
      sprites.push(new WT.Sprite(sheet, x * tileWidth, y * tileHeight,
                                 tileWidth, tileHeight));
    }
  }

  let canvas = document.getElementById("testCanvas");
  let game = new WT.Game(cellsX, cellsY, tileWidth, tileHeight,
                         WT.CoordSystem.Cartisan, canvas, sprites,
                         4); // 4 = middle tile

  let raised = [];
  raised.push(game.addTerrain(1, 2, 0, new WT.StaticGraphicsComponent(6)));
  raised.push(game.addTerrain(2, 2, 0, new WT.StaticGraphicsComponent(7)));
  raised.push(game.addTerrain(3, 2, 0, new WT.StaticGraphicsComponent(8)));

  let camera = new WT.Point(2 * tileWidth, 2 * tileHeight);
  game.update(camera);
  console.log("done");
}
