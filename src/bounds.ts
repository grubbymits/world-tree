...
export class EntityBounds {
  ...
  private static data: Float64Array = new Float64Array(
    this.MAX_ENTITIES * this.NUM_ELEMENTS * 2
  );
  ...
  static contains(id: number, p: Point3D): boolean {
    if (p.x < this.minX(id) || p.y < this.minY(id) || p.z < this.minZ(id)) {
      return false;
    }

    if (p.x > this.maxX(id) || p.y > this.maxY(id) || p.z > this.maxZ(id)) {
      return false;
    }
  ...
  static intersects(ida: number, idb: number): boolean {
    if (this.minX(idb) > this.maxX(ida) || this.maxX(idb) < this.minX(ida)) {
      return false;
    }

    if (this.minY(idb) > this.maxY(ida) || this.maxY(idb) < this.minY(ida)) {
      return false;
    }

    if (this.minZ(idb) > this.maxZ(ida) || this.maxZ(idb) < this.minZ(ida)) {
      return false;
    }
  ...
  static intersectsBounds(id: number, min: Point3D, max: Point3D): boolean {
    if (min.x > this.maxX(id) || max.x < this.minX(id)) {
      return false;
    }

    if (min.y > this.maxY(id) || max.y < this.minY(id)) {
      return false;
    }

    if (min.z > this.maxZ(id) || max.z < this.minZ(id)) {
      return false;
    }
  ...
  static axisOverlapX(ida: number, idb: number): boolean {
    if (
      (this.minX(idb) >= this.minX(ida) && this.minX(idb) <= this.maxX(ida)) ||
      (this.maxX(idb) >= this.minX(ida) && this.maxX(idb) <= this.maxX(ida))
    ) {
      return true;
    }
    return false;
  ...
  static axisOverlapY(ida: number, idb: number): boolean {
    if (
      (this.minY(idb) >= this.minY(ida) && this.minY(idb) <= this.maxY(ida)) ||
      (this.maxY(idb) >= this.minY(ida) && this.maxY(idb) <= this.maxY(ida))
    ) {
      return true;
    }
    return false;
  ...
  static axisOverlapZ(ida: number, idb: number): boolean {
    if (
      (this.minZ(idb) >= this.minZ(ida) && this.minZ(idb) <= this.maxZ(ida)) ||
      (this.maxZ(idb) >= this.minZ(ida) && this.maxZ(idb) <= this.maxZ(ida))
    ) {
      return true;
    }
    return false;
