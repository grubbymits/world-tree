...
export class Vector2D {
  constructor(private readonly _x: i32,
              private readonly _y: i32) {
    Object.freeze(this);
  }
  get x(): i32 { return this._x; }
  get y(): i32 { return this._y; }

  dot(other: Vector2D): number {
    const x: i32 = this.x * other.x;
    const y: i32 = this.y * other.y;
    return x + y;
  }
  ...
  angle(other: Vector2D): f32 {
    let x: i32 = this.x * other.y - other.x * this.y;
    let y: i32 = this.dot(other);
...
export class Segment2D {
  ...
  on(p: Point2D): bool {
    const dxc: i32 = p.x - this.p0.x;
    const dyc: i32 = p.y - this.p0.y;
    const dxl: i32 = this.p1.x - this.p0.x;
    const dyl: i32 = this.p1.y - this.p0.y;
    return (dxc * dyl - dyc * dxl) == 0;
  }

  intersects(other: Segment2D): bool {
  ...
  distance(p: Point2D): f32 {
    // https://www.geomalgorithms.com/a02-_lines.html
    const vl: i32 = this.p0.x * this.p1.y - this.p1.x * this.p0.y;
    const w: i32 = this.p0.x * p.y - p.x * this.p0.y;
    const u: f32 = 1 / Math.sqrt(Math.pow(this.p1.x - this.p0.x, 2) +
...
export class Point3D {
  constructor(private readonly _x: f32,
              private readonly _y: f32,
              private readonly _z: f32) {
    Object.freeze(this);
  }

  get x(): f32 { return this._x; }
  get y(): f32 { return this._y; }
  get z(): f32 { return this._z; }
...
export class Vector3D {
  ...
  add(other: Vector3D): Vector3D {
    const x: f32 = this.x + other.x;
    const y: f32 = this.y + other.y;
    const z: f32 = this.z + other.z;
    return new Vector3D(x, y, z);
  }

  mulScalar(factor: f32): Vector3D {
    const x: f32 = this.x * factor;
    const y: f32 = this.y * factor;
    const z: f32 = this.z * factor;
    return new Vector3D(x, y, z);
  }

  dot(other: Vector3D): f32 {
    const x: f32 = this.x * other.x;
    const y: f32 = this.y * other.y;
    const z: f32 = this.z * other.z;
    return x + y + z;
  ...
  cross(other: Vector3D): Vector3D {
    const x: f32 = this.y * other.z - this.z * other.y;
    const y: f32 = this.z * other.x - this.x * other.z;
    const z: f32 = this.x * other.y - this.y * other.x;
    return new Vector3D(x, y, z);
  }
  ...
  angle(other: Vector3D): f32 {
    // https://www.jwwalker.com/pages/angle-between-vectors.html
    const x: f32 = this.cross(other).mag();
    const y: f32 = this.dot(other);
    return Math.atan2(x, y);
  }

  absMin(other: Vector3D): Vector3D {
    const x: f32 = Math.abs(this.x) < Math.abs(other.x) ? this.x : other.x;
    const y: f32 = Math.abs(this.y) < Math.abs(other.y) ? this.y : other.y;
    const z: f32 = Math.abs(this.z) < Math.abs(other.z) ? this.z : other.z;
    return new Vector3D(x, y, z);
  }

  equal(other: Vector3D): bool {
...
export class BoundingCuboid {
  ...
  get minX(): f32 { return this.minLocation.x; }
  get minY(): f32 { return this.minLocation.y; }
  get minZ(): f32 { return this.minLocation.z; }
  get maxLocation(): Point3D { return this._maxLocation; }
  get maxX(): f32 { return this.maxLocation.x; }
  get maxY(): f32 { return this.maxLocation.y; }
  get maxZ(): f32 { return this.maxLocation.z; }
  get centre(): Point3D { return this._centre; }
  get bottomCentre(): Point3D { return this._bottomCentre; }
  get width(): f32 { return this._dimensions.width; }
  get depth(): f32 { return this._dimensions.depth; }
  get height(): f32 { return this._dimensions.height; }
  get dimensions(): Dimensions { return this._dimensions; }

  set centre(centre: Point3D) {
    this._centre = centre;
    const width: f32 = this.width / 2;
    const depth: f32 = this.depth / 2;
    const height: f32 = this.height / 2;

    let x: f32 = centre.x - width;
    let y: f32 = centre.y - depth;
    let z: f32 = centre.z - height;
  ...
  contains(location: Point3D): bool {
  ...
  containsBounds(other: BoundingCuboid): bool {
    return this.contains(other.minLocation) &&
           this.contains(other.maxLocation);
  }

  intersects(other: BoundingCuboid): bool {
  ...
  insert(other: BoundingCuboid): void {
    if (this.containsBounds(other)) {
      return; // nothing to do.
    }

    let minX: f32 = other.minLocation.x < this.minLocation.x ?
      other.minLocation.x : this.minLocation.x;
    let minY: f32 = other.minLocation.y < this.minLocation.y ?
      other.minLocation.y : this.minLocation.y;
    let minZ: f32 = other.minLocation.z < this.minLocation.z ?
      other.minLocation.z : this.minLocation.z;
    let maxX: f32 = other.maxLocation.x > this.maxLocation.x ?
      other.maxLocation.x : this.maxLocation.x;
    let maxY: f32 = other.maxLocation.y > this.maxLocation.y ?
      other.maxLocation.y : this.maxLocation.y;
    let maxZ: f32 = other.maxLocation.z > this.maxLocation.z ?
    ...
    const width: f32 = (max.x - min.x) / 2;
    const depth: f32 = (max.y - min.y) / 2;
    const height: f32 = (max.z - min.z) / 2;
...
export class Vertex3D {
  ...
  distance(p: Point3D): f32 {
    const sn: f32 = -this.normal.dot(p.vec_diff(this.point));
    const sd: f32 = this.normal.dot(this.normal);
    const sb: f32 = sn / sd;
    const closest: Point3D = p.addScalar(sb).mul(this.normal);
    const d: f32 = p.vec_diff(closest).norm();
    return d;
  }
  ...
  intersects(begin: Point3D, end: Point3D): Point3D|null {
    ...
    const a: f32 = -this.normal.dot(w0);
    const b: f32 = this.normal.dot(dir);
...
export abstract class Face3D {
  ...
  abstract intersects(end: Point3D): bool;

  abstract vertices(): Array<Vertex3D>;
}

class TriangleFace3D extends Face3D {
  private readonly _uDotv: f32;
  private readonly _uDotu: f32;
  private readonly _vDotv: f32;
  private readonly _denominator: f32;
  ...
  intersects(i: Point3D): bool {
    ...
    let w: Vector3D = i.vec_diff(this.vertex.point);
    let u = this.vertex.u;
    let v = this.vertex.v;
    const wDotv: f32 = w.dot(v);
    const wDotu: f32 = w.dot(u);
...
export class IntersectInfo {
  ...
  get theta(): f32 { return this._theta; }
}
  ...
  obstructs(begin: Point3D, end: Point3D): IntersectInfo|null {
    for (let face of this._faces) {
      ...
      if (i != null && face.intersects(i)) {
        let v0: Vector3D = i.vec_diff(begin);
        let v1: Vector3D = face.plane.normal;
        let theta: f32 = v0.angle(v1);
